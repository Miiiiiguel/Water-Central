          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative rounded-2xl p-8 border flex flex-col ${
                plan.highlighted
                  ? 'bg-gradient-to-b from-primary to-blue-800 text-white border-transparent shadow-glow-lg lg:-translate-y-4'
                  : 'bg-white border-gray-200 shadow-premium'
              }`}
            >
              {plan.highlighted && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 bg-cyan-400 text-primary text-xs font-bold rounded-full shadow-glow">
                  {language === 'es' ? 'Más elegido' : 'Most chosen'}
                </span>
              )}
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center font-bold mb-6 ${
                  plan.highlighted ? 'bg-white/20 text-white' : 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white'
                }`}
              >
                {plan.step}
              </div>
              ...
              <p className={`text-2xl font-bold mb-6 ${plan.highlighted ? 'text-cyan-200' : 'text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-blue-600'}`}>
                {language === 'es' ? plan.price.es : plan.price.en}
              </p>
              <ul className="space-y-3 mb-8 flex-1">
                {(language === 'es' ? plan.features.es : plan.features.en).map((feature, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <Check size={18} className={`flex-shrink-0 mt-0.5 ${plan.highlighted ? 'text-cyan-300' : 'text-cyan-600'}`} />
                    <span className={`text-sm ${plan.highlighted ? 'text-cyan-50' : 'text-muted-foreground'}`}>{feature}</span>
                  </li>
                ))}
              </ul>
              <Button
