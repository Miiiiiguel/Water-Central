...
function ProductWidget({ platform, side }: { platform: 'tiktok' | 'amazon'; side: 'left' | 'right' }) {
  ...
  return (
    <div
      className={`hidden 2xl:block absolute top-1/2 -translate-y-1/2 ${side === 'left' ? 'left-6 -rotate-2' : 'right-6 rotate-2'} w-64 bg-white/95 backdrop-blur-md rounded-2xl shadow-premium-lg border border-white/60 p-4 z-20 animate-float`}
